...
| **Desktop / tablet responsive layout** | `4ae1ccc`→`eaf2f11` + _uncommitted (8 pages)_ | The mobile-first 430px app now adapts to tablet + desktop. New `components/Sidebar.tsx` left rail replaces the bottom nav at the `lg` breakpoint (≥1024px); new `hooks/useMobile.tsx` exports `useIsMobile()` (<768) and `useIsDesktop()` (≥1024, SSR-safe — returns `false` on first render so mobile is the fallback until the media query resolves, no flicker). `App.tsx`'s `AppLayout` renders the sidebar beside a content frame that widens past mobile: `max-w-[430px] lg:max-w-[1100px] xl:max-w-[1440px] 2xl:max-w-[1760px]`. **The frame widens automatically, but each page must opt into the width** — so a per-page sweep adds pure-Tailwind `lg:`/`xl:`/`2xl:` treatments. Mobile (unprefixed) classes are left untouched (no 430px regression) and no JS breakpoint hooks are used inside pages (no first-render flash). **Wave 1 (committed `4ae1ccc`→`eaf2f11`):** Dashboard, Projects, Company, Profile, StaffDirectory, StaffProfile, CustomerDatabase, QuotationList, QuotationDetail, CreateProject, CreateQuotation, KPIPerformance, LeaveManagement, Recruitment, Announcements — content padding grows (`lg:px-8 lg:py-7 lg:space-y-6`), card stacks become 2-/3-col grids (`lg:grid lg:grid-cols-2 xl:grid-cols-3`). The 3 action overlays (`ConvertInquiryDialog`, `MarkCollectedSheet`, `SignDocumentSheet`) render as centred modals on desktop instead of bottom sheets, gated on `useIsDesktop()`. **Wave 2 (this change, the 8 pages that still had zero responsive classes):** ProjectDetail + CustomerDetail → two-column `lg:grid-cols-[1fr_360px]` (primary content + metadata side rail); CalendarPage → calendar left + agenda/events rail right (`1fr_340px`), roomier day cells; PerformanceReport / Checkpoints / Reminders → KPI strips go 4-across, card lists become multi-col grids, related sections sit side-by-side; AuditLog → entry cards go row-oriented (table-like) on `lg` with an inline timestamp, filters 2-col, list 2→3 col; **ClientPortal** (public, renders outside `AppLayout` so it doesn't inherit the frame) → widened its own `Frame` cap to `lg:max-w-[900px] xl:max-w-[1100px]` and floats as a card, with multi-col photo (`lg:grid-cols-4 xl:grid-cols-5`), payment, and signature grids. Layout/className only — **zero design-token (oklch) changes** (verified: no net-new colours, only reuse), zero logic/hook/state changes. TS check (`bun run check`) + production build (`vite build`) both exit 0. _(Follow-up: the unused `ManusDialog` scaffold component was deleted — see below.)_ |
| **Delete dead `ManusDialog`** | _uncommitted_ | Removed `client/src/components/ManusDialog.tsx` — a vestigial "Login with Manus" Radix dialog from the original Manus scaffold (same lineage as the `vitePluginManusRuntime` code dropped in Phase 0F). Imported nowhere (`grep -rn ManusDialog client/` → only its own file). Pure dead-code removal; TS check + build unaffected. |
| **Staff & project editing + real profile data** | _uncommitted_ | Closes the "everything is read-only" gap (staff directory + project header). **Hooks:** new `useUpdateStaff` + `useUpdateProject` in `queries.ts` (mirror the `useUpdatePayment` pattern — partial `TablesUpdate` payloads, DD/MM/YYYY→ISO at the boundary, cache invalidations on `qk.staff`/`qk.staffOne` and `qk.projects`/`qk.project`). **Gating helpers:** `isAdminTier`, `canEditStaffRow(role, isOwnRow)` (admin → any row, anyone → own row), `canEditProject` (alias of OPS-tier `canEditPayments`). **UI:** new `EditStaffSheet` (dual-mode — admins edit name/job/dept/role/status/phone/WhatsApp; self-editors only phone + WhatsApp opt-in; `email` never editable) and `EditProjectSheet` (name/status/priority/location/budget/size/dates/client info/description) — both follow the `MarkCollectedSheet` bottom-sheet↔desktop-modal pattern. Wired an "Edit" affordance into the StaffProfile hero header (shown per `canEditStaffRow`) and the ProjectDetail header `rightAction` (shown per `canEditProject`). **Profile data fix:** StaffProfile now shows real `role` + `whatsapp_opt_in`, and the Leave tab's previously hardcoded totals (16/14 + a fabricated "Replacement Leave 0/3" row) were replaced — real remaining balances against a documented `LEAVE_ENTITLEMENT` policy constant, fake row removed. **Security hardening:** migration `20260602000001_tier1_5_staff_update_guard.sql` adds a `BEFORE UPDATE` trigger (`guard_staff_sensitive_columns`) that blocks non-admins from changing `role`/`status`/`email`/`auth_user_id` on their own row — RLS is row-level only, so without it the `staff_update_self` policy would allow self-promotion via direct API. The trigger bypasses the system/service-role context (`auth.uid() is null`) so the auth-linking trigger still works. ⚠️ **Apply this migration in the Supabase SQL Editor before relying on self-edit in production.** TS check + production build exit 0. |
| **Fix: stale lazy-chunk recovery** | _uncommitted_ | After a deploy, Vite rotates the hashed chunk filenames and deletes the old ones, so an already-open tab navigating to a lazy route would fetch a now-404 chunk and crash with "Failed to fetch dynamically imported module" (observed on `/projects/PRJ001` → `ProjectDetail-*.js`). Added a `lazyWithReload` wrapper in `App.tsx` around all 18 lazy route imports: on a chunk-fetch failure it triggers one full `window.location.reload()` (fetching fresh HTML + current chunk names), guarded by a `sessionStorage` flag so it can't loop — if the import still fails after the reload it rethrows to the ErrorBoundary as before. The flag clears on any successful load so each future deploy can self-recover. No behaviour change on the happy path. TS check + build exit 0. |
...
**Migration to apply (Tier 1.5):**
- `supabase/migrations/20260602000001_tier1_5_staff_update_guard.sql` — paste + run in the Supabase SQL Editor. Adds the `guard_staff_sensitive_columns` trigger that stops non-admins from changing `role`/`status`/`email` on their own staff row (defence-in-depth behind the `staff_update_self` RLS policy now that self-edit UI exists). The app works without it, but apply it before letting non-admin staff edit their own profile in production.
...
- ~~**No mutation hooks** for editing staff records or project metadata.~~ ✅ Resolved (Tier 1.5) — `useUpdateStaff` / `useUpdateProject` + `EditStaffSheet` / `EditProjectSheet`, gated to admin-or-self (staff) and OPS tier (project). `staff.email` remains intentionally non-editable (magic-link key). Assignee reassignment (designer/pm via a staff picker) is still a future enhancement.
...
- ~~**`ManusDialog` is dead code.**~~ ✅ Resolved (Tier 1.5) — deleted.
