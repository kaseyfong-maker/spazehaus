...
- **`HERO_BG` still points at the external Manus CDN.** 13 pages hard-code a `files.manuscdn.com` image URL (3 distinct images, used as an `AppHeader` `bgImage` CSS background) — see `grep -rn "manuscdn" client/src`. If that CDN disappears, every page header loses its background. Self-host: drop the 3 images in `client/public/hero/` (Vite serves `client/public` at `/`), reference `/hero/<name>.jpg`, and ideally DRY the constant into one shared module instead of redeclaring it per page. (Tier 1.5 item.)
- **`ManusDialog` overlay is not desktop-aware.** The other 3 overlays became centred modals on `lg` via `useIsDesktop()`; `ManusDialog` still presents as a mobile bottom sheet on desktop. Small follow-up to match the pattern.
